...
This plugin does not own MCP route schemas. Live tool names and request shapes come from `ecartpay-api` (`https://mcp.ecartpay.com/mcp`) after `npm run scan` in `ecart-payment`. Docs discovery stays on MCP `ecartpay` (`https://docs.ecartpay.com/mcp`). Do not invent endpoints or fields.

## Workflow

1. Inspect the MCP server codebase from the current working directory. If this repo only declares MCP URLs, inspect the live `ecartpay-api` / `ecartpay` tool descriptors and note that annotation source of truth is `ecart-payment`.
...
When the MCP implementation lives in `ecart-payment` and this plugin only lists URLs, report missing hints and recommended values, then generate JSON only if the developer asked to produce the import file for the OpenAI portal using those recommended values.
