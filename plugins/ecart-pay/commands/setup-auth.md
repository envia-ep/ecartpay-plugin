...
1. If `ecartpay-api` is connected: call `ecartpay_session`. Authenticated session is enough — the HTTP MCP calls the API with Basic merchant keys (same as stdio `createApiClient`). Send `x-ecartpay-mode` matching `ECARTPAY_MODE`. Sandbox on this server only works when sandbox keys are linked; otherwise say so and use Configure sandbox keys + docs MCP `ecartpay`.
...
5. Output: mode, API host, auth path (`ecartpay-api` session vs JWT). Next: `/create-order` or `/create-payment-link`.
